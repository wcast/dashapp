...
use public\libs\ckeditor\plugins\filemanager\include\imageLib;
...
if ( ! function_exists('response'))
{
	...
	* @param array  $headers
	*
	* @return \public\libs\ckeditor\plugins\filemanager\include\Response|\Illuminate\Http\Response
	*/
...
function deleteFile($path,$path_thumb,$config){
	if ($config['delete_files']){
		...
		if($ftp){
			...
			}catch(\public\libs\ckeditor\plugins\filemanager\include\FtpException $e){
				return;
			}
		}else{
			if (file_exists($path)){
...
function ftp_con($config){
	if(isset($config['ftp_host']) && $config['ftp_host']){
		...
		$ftp = new \public\libs\ckeditor\plugins\filemanager\include\FtpClient();
		try{
