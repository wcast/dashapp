            ...
            if (count($this->imoveis)) {
                ...
                foreach ($this->imoveis as $imovel) {
                    ...
                    if (is_null($existe)) {
                        ...
                        $existe->update($imovel);
                        Log::info("Imóvel: {$existe['Codigo']} atualizado existe.");
                    }
                }
    ...
    public function checkProperties()
    {
        ...
        foreach ($properties as $property) {
            ...
            if (!isset($existe_imovel['Codigo'])) {
                ...
                DB::table('properties_exclusivits')->where('Codigo', $property->Codigo)->delete();
        ...
        if (isset($return['total'])) {
            ...
            for ($i = 1; $i <= $paginas; $i++) {
                ...
                foreach ($return as $corretor) {
                    ...
                    $equipes = [];
                    foreach ($corretor['Equipe'] as $equipe){
                        $equipes[] = $equipe['Nome'];
                    }
                    try {
                        Log::info("Corretor: {$corretor['CRECI']}.");
                        if($this->in_array_r(['CR1','CR2','HP','LA'], $equipes, true)){
                            unset($corretor['Inativo']);
                            unset($corretor['Equipe']);
                            unset($corretor['Equipe_Codigo']);
                        ...
                        Log::info("Corretor: {$e->getMessage()}");
                    }
                }
            }
        }
